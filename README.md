...
| **分揀通道** | 10 個固定位置(L1–L5 / R1–R5),指派物流、貼標人員與本機印表機(`direct_print` 模式送印依此);支援手機遙控暫停 / 跳過本輪 |
| **網頁版** | 桌面版的頁面用瀏覽器也能開(同一份前端、同一批後端邏輯)。**掃描列印、自動印單、入倉驗單除外** —— 這三頁要接掃描槍、用中介機的印表機出單,只在桌面版提供。現場網路內免登入;開放外部連線後,主管在外面輸入密碼即可查看查詢記錄、統計與現場狀況 |
| **手機遙控分揀** | 同區網手機開網頁版即可暫停 / 恢復 / 跳過某通道,即時同步桌面 GUI(舊網址 `/control` 仍會自動導向) |
...
> ⚠️ **上述端點只接受內網來源**。工控機與中介機同網段時行為不變、不需修改;來源不在內網網段時一律回 403。
> 中介機不採信 `X-Forwarded-For` 等標頭,判斷以實際連線來源為準。

> **網頁版**:`GET /` 開整套操作介面;`POST /rpc/{command}` 資料通道、`GET /events/stream` 事件推播(SSE)、
> `POST /auth/login` 登入。舊網址 `/control`、`/board` 導向對應頁面,現場貼的 QR 不必重貼。

---

## 網頁版與對外開放

桌面版的頁面用瀏覽器也能開,包含查詢記錄、各項統計、分揀看板與現場監控。
**掃描列印、自動印單、入倉驗單三頁只在桌面版提供**(要接掃描槍、用中介機的印表機出單);
網頁版的導覽列不會列出,直接輸入網址也會被帶回首頁。網址就是中介機的
`http://{中介機IP}:18080/` —— 桌面 App 右上角的手機圖示會列出可用網址與 QR。

**存取控制**(設定於「伺服器設定 → 網頁存取」):

| 連線來源 | 需要密碼嗎 | 可以做什麼 |
|---|---|---|
| 現場網路內(預設含 `192.168.*`、`10.*`、`172.16-31.*`) | 不用 | 全部功能 |
| 現場網路以外 | 要 | 全部功能(與坐在現場相同) |
| 現場網路以外連工控機端點 | — | 一律拒絕 |

要讓主管從外面連進來,需要三件事:
1. 在「網頁存取」設定**共用密碼**(至少 8 字);沒設密碼的話外面沒有人進得來。
2. 打開**開放外部連線**開關(預設關閉)。
3. 在路由器把 18080 對外開放,並準備一個 DDNS 網址。

> 🚨 **目前連線沒有加密。** 從外面連進來時,密碼與面單上的收件人姓名、電話、地址都是以未加密的方式
> 在網路上傳送,同一段網路上的人有機會看到(公共 Wi-Fi 尤其要注意)。建議只在需要時打開對外開關。

安全性上已具備:密碼以 argon2 雜湊保存、連續輸錯 5 次鎖 15 分鐘、登入有效期 8 小時(可調)、
登入成功與失敗都寫進事件記錄。
