...
commit 見 git log(tag `v1.1.0` 第三次打:第二次 `7675140` 之後又補了看板頁;第一次打在 `ab95774` 的 tag / 草稿 release / CI 已取消刪除,
...
- `/board` 同樣接回獨立看板頁(`server/board_page.html`,靠 `/api/channels` + `/board/stream`),
  並依使用者要求:**拿掉標題列**(語系與全螢幕改成右上角浮動小鈕、斷線才顯示紅色提示)、
  **加全螢幕鈕**(Fullscreen API;iOS Safari 不支援整頁全螢幕 → 按鈕自動隱藏,改靠 `apple-mobile-web-app-capable`
  加入主畫面開成無網址列)。分揀通道頁 `DisplayLauncher` 的 `web-path` 改 `/board`。
  Chrome 1920×1080 實測:無 header、全螢幕可進入、切語系連「等待包裹」一起換、斷線橫幅會出現。
- `CLAUDE.md` 已寫明「不要把 /control、/board 改成導向完整網頁版」。
...
## 2026-09-10：網頁版不提供現場作業頁（v1.0.1，已發佈）

使用者確認:網頁版不會跑掃描出單。**掃描列印、自動印單、入倉驗單**三頁在網頁版隱藏。

- 依據(查過程式碼,不是憑名稱):這三頁各有 8~11 處掃描輸入、2~6 處本機列印。
  另外三頁看似同類 —— 面單預產、包裹入廠、司機派工 —— **掃描輸入與本機列印都是 0**
  (預產是叫中介機去雲端批次下載;入廠/派工是表單送雲端),網頁版照常提供。
- 做法:`desktopOnly: true` 標在導覽列與路由兩處。導覽列依環境過濾,整組被拿掉的分組標題
  (「訂單列印」)一併移除;路由守衛擋直接輸入網址與舊書籤,帶回首頁。
- 實測:網頁版導覽列 25 → 21 項(3 頁 + 1 個空分組標題);三個網址直接輸入都落回 `#/`;
  **桌面版 25/25 一項沒少**(esbuild 打包 navConfig 分別模擬兩種環境驗證)。
- 兩處標記的一致性加了測試,並以「只標一邊」的破壞情境確認會擋下。
- **已發佈 v1.0.1**(2026-09-10 10:37 UTC,commit `258175b`):CI 六個 job 全綠含 `verify-assets`,
  九項產物齊全,`latest.json` 匿名可取。v1.0.0 的 release notes 寫的是 v1.0.0 當下的行為
  (當時確實含入倉驗單),不需回頭改。
